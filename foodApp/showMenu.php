...
<div class="container-fluid d-flex p-0 bg-dark">

<?php include "sidebar.php";?>
<form action='' method = 'post' class="w-100">

<h1 class="text-center bg-dark text-light m-0 p-3 ">
 ...
 include "connect.php";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        $id = $row['order_no'];
      echo "<tr><td>".$row['order_no']. "</td><td>". $row["type"]. "</td><td>". $row["name"]. "</td><td>".  $row["price"]."</td><td>". $row["image"]."</td>
      <td>
...
$edit = $_POST['edit'];
$_SESSION['edit'] = $edit;
echo "<script> location.replace('editMenu.php'); </script>";
...
if (isset($_POST['delete'])){
  ...
  echo "<script> location.replace('deleteMenu.php'); </script>";
}
